...
# training_model = {"RF", "LSTM", "SVM", "BT", "no"}
training_model = "LSTM"
...
if training_model == "RF" or training_model == "BT":
    ...
    model_1 = Sequential()
    # 124848
    model_1.add(Embedding(6823, 32, input_length=maxWordCount))  # to change words to ints
    model_1.add(LSTM(10))
    ...
    tensorboard = keras.callbacks.TensorBoard(log_dir='./logs/log_26', histogram_freq=0, write_graph=True,
                                              write_images=False)
    checkpointer = ModelCheckpoint(filepath="./weights/weights_26.hdf5", verbose=1, save_best_only=True,
                                   monitor="val_loss")
    ...
    history = model_1.fit(train_vec, train_data_y, epochs=epochs, batch_size=batch_size, verbose=1,
                          validation_data=(test_vec, test_data_y),
                          callbacks=[tensorboard, reduce_lr, checkpointer, earlyStopping])

    print("=============================== Score =========================================")

    scores = model.evaluate(test_vec, test_data_y, verbose=0)
